...
public static class ListBucketFilesEndpoint
{
    public static RouteHandlerBuilder MapBucketListFilesEndpoint(this IEndpointRouteBuilder endpoints)
    {
        return endpoints
            .MapPost("/List/{id:guid}/Folder/{folderid:guid}/Files", async (ISender mediator, [FromBody] PaginationFilter filter) =>
            {
                var response = await mediator.Send(new ListBucketFilesRequest(filter));
                return Results.Ok(response);
            })
            .WithName(nameof(ListBucketFilesEndpoint))
