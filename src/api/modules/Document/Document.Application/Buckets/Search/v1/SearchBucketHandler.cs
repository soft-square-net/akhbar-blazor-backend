...
public sealed class SearchBucketsHandler(
    ...
    : IRequestHandler<SearchBucketsCommand, PagedList<BucketResponse>>
{
    public async Task<PagedList<BucketResponse>> Handle(SearchBucketsCommand request, CancellationToken cancellationToken)
    {
        ...
        return new PagedList<BucketResponse>(items, request!.PageNumber, request!.PageSize, totalCount);
    }
}
